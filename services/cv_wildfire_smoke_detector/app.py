...
class WildfireDetector(ModelManager):
    ...
    def postprocess(self, predictions: torch.Tensor) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "smoke_probability": float(pred[1]),
                "has_smoke": bool(pred[1] > 0.5),
...
def process_message(message: Dict[str, Any]):
    ...
    try:
        ...
        message['wildfire_detection'] = results
        
        # Emit processed message
        event_producer.emit_event('wildfire_detections', message)
